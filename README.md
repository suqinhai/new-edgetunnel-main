...
| **FORCE_IPV4** | ❌ | `true` | 严格使用 IPv4；配置 PROXYIP 时先走 IPv4 反代入口，直连目标仅使用 A 记录，不回退到 IPv6。远端反代服务器自身的出站地址仍由其网络配置决定。 |
...
```bash
npx wrangler d1 create edgetunnel-access
```

将返回的 `database_id` 填入 `wrangler.toml` 的 `[[d1_databases]]`，取消该段注释，然后执行：
